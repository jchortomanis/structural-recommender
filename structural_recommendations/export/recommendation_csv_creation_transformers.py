...
# Read the recommendations list from the transformers model
transformers_recommendations_list = pd.read_csv('../../output/transformers_recommendations_list_n_3894.csv', header=None)       # the csv has been saved without headers
...
for i in range(len(all_entities)):
    most_similar_text_indices = transformers_recommendations_list.iloc[i][1:]                      # get the recommended sequence numbers for the 'i'th item of the list
...
all_recommendations.to_csv('output/all_recommendations_transformers.csv')
