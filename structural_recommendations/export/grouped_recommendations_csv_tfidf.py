...
tfidf_recommendations_list = pd.read_csv('../../output/tfidf_all_recommendations_list.csv', header=None)       # the csv has been saved without headers
...
for i in range(len(all_entities)):
    print("i:", i)
    most_similar_text_indices = tfidf_recommendations_list.iloc[i][1:]      # [1:] because we do not want the first recommendation (which we assume that is the landing page itself)
...
grouped_recommendations.to_csv('output/tfidf_grouped_recommendations.csv')
